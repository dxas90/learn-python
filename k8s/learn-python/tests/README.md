# Helm Unit Tests — learn-python Chart

This directory contains unit tests for the `learn-python` Helm chart using the `helm-unittest` plugin.
...
```bash
helm unittest k8s/learn-python
```
...
```bash
helm unittest -f 'tests/deployment_test.yaml' k8s/learn-python
helm unittest -f 'tests/service_test.yaml' k8s/learn-python
```

- Run in debug mode (verbose rendering):

```bash
helm unittest -d k8s/learn-python
```

- Generate JUnit report for CI:

```bash
helm unittest --output-type JUnit --output-file test-results.xml k8s/learn-python
```
  ...
  - it: should set custom image tag
    ...
    asserts:
      - equal:
          ...
          value: ghcr.io/dxas90/learn-python:v1.0.0
```
...
- name: Run Helm Unit Tests
  run: helm unittest k8s/learn-python --output-type JUnit --output-file test-results.xml
